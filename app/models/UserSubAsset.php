 ...
 * @method static Builder|UserSubAsset whereAssetId($value)
 * @method static Builder|UserSubAsset whereUserAssetId($value)
 * @method static Builder|UserSubAsset whereUserId($value)
...
class UserSubAsset extends Model
{
    ...
    public function is_in_form($form_field_id, $is_pic = false, $gallery = false) {
        ...
        $uId = Auth::user()->id;
        if($is_pic)
            return (DB::select("select count(*) as countNum from user_sub_assets u, forms f, form_fields ff where u.asset_id = f.asset_id " .
